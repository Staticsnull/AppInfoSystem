									...
									<select name="queryFlatformId" class="form-control">
										<c:if test="${flatFormList != null }">
											...
											<c:forEach var="dataDictionary" items="${flatFormList }">
												<option
													<c:if
															test="${dataDictionary.valueId == queryFlatformId}">selected="selected"</c:if>
													value="${dataDictionary.valueId }">${dataDictionary.valueName }</option>
											</c:forEach>
										</c:if>
									</select>
						...
						<td> <div class="btn-group">
	                      ...
	                      <li><c:choose>
	                      		<c:when test="${appinfo.status == 2 || appinfo.status == 5 }">
									 ...
									 data-original-title="恭喜您,审核已经通过,您可以点击上架发布您的APP"
									 data-toggle="tooltip" data-placement="top" title="" >上架</a>
	                      		</c:when>
	                      		<c:when test="${appinfo.status == 4 }">
									 ...
									 appsoftwarename=${appinfo.softwareName } data-toggle="tooltip" 
									  data-original-title="您可以点击下架来停止发布您的APP，市场将不提供APP的下载"
									 data-placement="top"  title="">下架</a>
	                      		</c:when>
	                      	</c:choose></li>
	                      	<!--新增版本链接  -->
